...
contract TempoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STempo }
    ...
    address public sTempo;
    uint public sTempoQueue; // Delays change to sTempo address
    ...
    constructor (
       ...
       /* isLiquidityToken[ _TempoMIM ] = true;
       liquidityTokens.push( _TempoMIM ); */

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for Tempo
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Tempo needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Tempo ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Tempo for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITempoERC20( Tempo ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTempo ).balanceOf( msg.sender ); // Can only borrow against sTempo held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Tempo
        @param _amount uint
     */
    function repayDebtWithTempo( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITempoERC20( Tempo ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Tempo, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Tempo ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Tempo ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Tempo valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Tempo decimals
            value_ = _amount.mul( 10 ** IERC20( Tempo ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STempo ) { // 9
            sTempoQueue = block.number.add( blocksNeededForQueue );
        } else return false;
