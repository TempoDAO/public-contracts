...
contract LPTempoBondDepository is Ownable {
    ...
    address public immutable Tempo; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints Tempo when receives principle
    ...
    struct Bond {
        uint payout; // Tempo remaining to be paid
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Tempo ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) Tempo
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( Tempo ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Tempo ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Tempo ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Tempo ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Tempo ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Tempo supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( Tempo ).totalSupply();
    ...
    /**
     *  @notice calculate amount of Tempo available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Tempo) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != Tempo );
